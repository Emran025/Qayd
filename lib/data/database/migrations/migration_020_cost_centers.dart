...
''');
    ...
    // ── 7. Seed Default Categories & Dimensions ───────────────────────────
    ...
    for (final name in spatialItems) {
      await db.insert('cost_center_dimensions', {
        'id': uuid(),
        'name': name,
        'category': 'spatial',
        'cost_center_id': null,
    ...
    final individualItems = ['الزوجة', 'الأولاد', 'الوالدان', 'شخصي'];
    for (final name in individualItems) {
      await db.insert('cost_center_dimensions', {
        'id': uuid(),
        'name': name,
        'category': 'individual',
    ...
    final projectItems = ['مشروع عام'];
    for (final name in projectItems) {
      await db.insert('cost_center_dimensions', {
        'id': uuid(),
        'name': name,
        'category': 'project',
